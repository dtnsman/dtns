 ...
 * @LastEditTime:
 -->
 <template>
    <div style="width: 100%;height: 100%;padding:0px;margin: 0px;background-repeat: no-repeat;background-size: cover;" ref="rtaudioroombody">
      ...
      <div :style="content_style">
        <img :src="imgSrc" width="150px" height="150px" style="border-radius: 75px;border:1px solid #ffffff"/>
...
//   import rtaudioroomimg from './rtaudioroom.jpg'
  import rtaudioroomJson  from './rtaudioroom.json'
  const rtaudioroom_channel_name = 'rtaudioroom-channel'
  const rtaudioroomimg = rtaudioroomJson.rtaudioroomimg
  // const micimg= rtaudioroomJson.micimg
  // const miccloseimg=rtaudioroomJson.miccloseimg
  const audioimg=rtaudioroomJson.audioimg
  ...
  const rtaudioroom_gifts_map= {'flower':'花','bicycle':'自行车','motorcycle':'摩托车','car':'小汽车',
    'roadster':'跑车','jets':'飞机'}
  const rtaudioroom_gifts_map_zh= {'花':'flower','自行车':'bicycle','摩托车':'motorcycle','小汽车':'car',
  '跑车':'roadster','飞机':'jets'}
  // const audiocloseimg=rtaudioroomJson.audiocloseimg

  export default {
    name: "RtAudioRoom",
    props: ["value"],
    components: { RtAudio , RoomList},
    data() {
      return {
        title:'RT语聊房',
    ...
    async created()
    {
      this.content_style = 'position: fixed;z-index: 99;display: flex;flex-wrap: wrap;align-items: center;justify-content: center; left:0px;right: 0px;top:50px;height:'+(document.documentElement.clientHeight-50)+'px;'
      // const os_type = this.gameW == document.documentElement.clientWidth ? 'mobile':'pc'
      this.msg_list_style='background-color: rgb(200,200,200,0);width:'+(document.documentElement.clientWidth)+'px;height:'+(document.documentElement.clientHeight-280)+'px;'
      ...
      const focusRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtchannel/focus',{channel:rtaudioroom_channel_name})
      if(!focusRet ||!focusRet.ret) return this.$toast('订阅频道失败！原因：'+(focusRet?focusRet.msg:'未知网络原因'))
      this.$toast('订阅频道成功')
    
    },
    mounted(){
      console.log('rtaudioroomimg:',rtaudioroomimg,this.h)
      this.$refs.rtaudioroombody.style.backgroundImage = `url(${rtaudioroomimg})`
      window.g_rtaudioroombody = this.$refs.rtaudioroombody

      if(window.rtaudioroomInfo && window.rtaudioroomInfo.room_id)
      {
        this.room_id = window.rtaudioroomInfo.room_id
        this.reqRoomInfo()
      }
      else{
        this.reqMatch()
    ...
    methods: {
          ...
          if(window.g_pop_event_bus)
          {
            ...
            this.event_func = function(data){
                ...
                else if(data.notify_type == 'gift')
                {
                  ...
                  if(!This.room_id) return false
                  if(data.xobj.room_id !=This.room_id) return false
                  This.addMsg({name:'系统消息',msg:data.xobj.name+'赠送了主播礼物-'+rtaudioroom_gifts_map[data.xobj.gift]+'！'})
                }
              }
            window.g_pop_event_bus.on(rtaudioroom_channel_name,this.event_func)
            window.g_pop_event_bus.on('rtaudioroom_into_room',this.intoRoom)
          }
        },
        async unbindEvent()
        {
          ...
          if(window.g_pop_event_bus)
          {
            window.g_pop_event_bus.removeListener(rtaudioroom_channel_name,this.event_func)
            window.g_pop_event_bus.removeListener('rtaudioroom_into_room',this.intoRoom)
          } 
          const unfocusRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtchannel/unfocus',{channel:rtaudioroom_channel_name})
        ...
        async reqMatch()
        {
          window.rtaudioroomInfo = null
          this.onLeave()
          // this.$toast('匹配棋局...')
          this.addMsg({name:'系统消息',msg:'RT语聊房，匹配中...'})
          const matchRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtaudioroom/match',{})
          if(!matchRet ||!matchRet.ret){
              return this.$toast('匹配失败！原因：'+(matchRet?matchRet.msg:'未知网络原因'))
          }
          else 
          {
            this.addMsg({name:'系统消息',msg:'RT语聊房，匹配成功！'})
            ...
            window.rtaudioroomInfo = this.roomInfo
        ...
        async reqCreate()
        {
          ...
          this.onLeave()
          // this.$toast('匹配棋局...')
          this.addMsg({name:'系统消息',msg:'RT语聊房，创建中...'})
          ...
          const createRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtaudioroom/create',{user_name,logo})
          if(!createRet ||!createRet.ret){
              return this.$toast('创建失败！原因：'+(createRet?createRet.msg:'未知网络原因'))
          }
          else 
          {
            this.addMsg({name:'系统消息',msg:'RT语聊房，创建成功！'})
        ...
        async reqRoomInfo()
        {
          ...
          const infoRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtaudioroom/info',{room_id:this.room_id})
        ...
        async reqRooms()
        {
          this.showList = true
          if(window.g_pop_event_bus)
            setTimeout(()=> window.g_pop_event_bus.emit('rtaudioroom_show_list',{}),100)
        },
        intoRoom(roomInfo)
        {
        ...
        {
          let msg = '可赠送礼物清单：'
          for(let i=0;i<rtaudioroom_gifts.length;i++)
          {
            const gift = zh ? rtaudioroom_gifts_map[ rtaudioroom_gifts[i] ] : rtaudioroom_gifts[i] 
            const price= rtaudioroom_prices[i]
            msg += gift+'$'+price+'；'
          }
        ...
        async send()
        {
          ...
          const sendRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtaudioroom/send',{msg,room_id:this.room_id,name:this.name})
          if(!sendRet ||!sendRet.ret){
              return this.$toast('发送失败！原因：'+(sendRet?sendRet.msg:'未知网络原因'))
          }
          if(rtaudioroom_gifts_map[msg] || rtaudioroom_gifts_map_zh[msg])
          {
            let gift = msg
            if(rtaudioroom_gifts_map_zh[msg]) gift = rtaudioroom_gifts_map_zh[msg]
            const sendGiftRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtaudioroom/gift/send',{gift,room_id:this.room_id,user_name:this.name})
