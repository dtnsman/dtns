...
export default {
  ...
  methods:{
    ...
    updateForm()
    {
        switch(this.step)
        {
            case 'ccy':
                ...
                break;
            case 'qrcode':
                const ccy = localStorage.getItem('goto-ccy')
                if(!ccy) {
    ...
    async queryAddrs()
    {
        ...
        const ret = await g_dtnsManager.run('dtns://web3:'+rpc_client.roomid+'/fintechsdk/addr',{ccy:ccy})
        console.log('queryAddrs-ret:',ret)
        if(!ret ||!ret.ret) return this.$toast('查询充值地址失败！')

        const formJson = Object.assign({},fintechsdkQRCodeJson)
        const templateJson = formJson.list[0].columns[0]
        const oldTemplateJsonStr =  JSON.stringify(templateJson) 
        ...
        if(addrs)
        {
            ...
            for(let i=0;i<addrs.length;i++)
            {
                ...
                const tmpJson = JSON.parse(oldTemplateJsonStr)//恢复模板
    ...
    async post(res)
    {
        ...
        else if(this.step == 'txid')
        {
            ...
            const ret = await g_dtnsManager.run('dtns://web3:'+rpc_client.roomid+'/fintechsdk/txid/notice',{txid:res.txid})
