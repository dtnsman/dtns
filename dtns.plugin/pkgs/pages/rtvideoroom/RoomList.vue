  ...
  const defaultlogo=rtvideoroomJson.defaultlogo
    export default {
        ...
        async created()
        {
            ...
            window.g_pop_event_bus.on('rtvideoroom_show_list',function(){
        ...
        methods: {
            ...
            async gotoItem(item)
            {
                ...
                if(window.g_pop_event_bus )
                window.g_pop_event_bus.emit('rtvideoroom_into_room',item)
            },
            async updateInfo()
            {
                const roomsRet = await window.g_dtnsManager.run('dtns://web3:'+window.rpc_client.roomid+'/rtvideoroom/rooms',{})
