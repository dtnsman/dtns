...
class DNALinkEngine
{
    ...
    async dtns_network_client_sync_files(records)
    {
        ...
        if(!g_sync_files_flag) return console.log('g_sync_files_flag is false')
        ...
        for(let i=0;i<rows.length;i++)
        {
            ...
            // if(txinfo.opcode.startsWith('rel') || txinfo.opcode=='map' || txinfo.opcode=='web3key' || txinfo.opcode=='fork')
            //     continue
            let opJSON = null;
            if(txinfo.opval.startsWith('{') && txinfo.token_x.startsWith('obj_'))
            try{
                console.log('dtns_network_client_sync_files-obj-info:',txinfo)
                opJSON = JSON.parse(txinfo.opval)
            }catch(ex){
                console.log('exception--txinfo.opval:',txinfo.opval)
                console.log('dtns_network_client_sync_files-json-parse-exception:'+ex,ex)
                continue
            }
            //&& (opJSON.fmt  || opJSON.file_kind || opJSON.img_kind ) 
           if(opJSON && opJSON.hash ||  txinfo.opcode == 'file')
           {
                ...
                try{
                    let downloadFlag = await new Promise((resolve)=>{
                        rpc_client.download(params,async function(data){
                            ...
                            let flag = await ifileDb.addData({key:hash,data:data.data})
                            resolve(flag)// true)
                            }else{
    ...
    async dtns_network_client_sync()
    {
        ...
        //     this.dtns_network_rtc_api.rtc_client.setPeerRefreshCallback(async function(){
                ...
                let userInfo = JSON.parse(localStorage.getItem('userInfo'))
                let client = await g_dtnsManager.connect('dtns://web3:'+web3name)
                let chainAllRecords = await g_dtnsManager.run('dtns://web3:'+web3name+'/chain/query',
                    {begin:0,len:10000000000,user_id:userInfo.user_id,s_id:userInfo.s_id,
                ...
                //判断是否提前同步了wallet-keys
                let walletInfo = null
                if(window.g_sync_web3keys_results)
                {
                    for(let i=0;i<g_sync_web3keys_results.length;i++){
                        if(g_sync_web3keys_results[i] && g_sync_web3keys_results[i].token == this.TOKEN_ROOT)
                        {
                            walletInfo = g_sync_web3keys_results[i]
                        }
                    }
                }
                console.log('sync-keys-walletInfo:',walletInfo,this.TOKEN_ROOT)

                //是否强制设置随机密钥 2023-6-25
                console.log('g_dnalink_node_auto_gen_keys:',g_dnalink_node_auto_gen_keys)
                if(window.g_dnalink_node_auto_gen_keys)
                {
                    let private_key = key_util.newPrivateKey() 
                    walletInfo = {private_key,
                        public_key : key_util.getPublicKey(private_key)}
                    console.log('engine-auto-gen-keys:',walletInfo)
                }

                if(walletInfo){
                    let saveWalletRet = await this.wallet_m.saveTokenSignKeys(this.TOKEN_ROOT,walletInfo.private_key,walletInfo.public_key)
                    console.log('saveWalletRet:',saveWalletRet)
                }
                console.log('dnalink_node_download_ended_save-typeof:',typeof This.dnalink_node_download_ended_save)
                if(typeof This.dnalink_node_download_ended_save == 'function')
                {
                    console.log('call dnalink_node_download_ended_save function')
                    This.dnalink_node_download_ended_save()
                }
                //请求设定websocket等
                ...
                client.setWs(link_token,async function(data){
                    ...
                    try{
                        ...
                        This.dtns_network_client_sync_files(data)
