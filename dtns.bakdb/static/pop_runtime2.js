...
class PopRuntime{
    ...
    constructor(context = null,safeFlag = true,commentRetainFlag = true,maxDeep = 1000100)
    {
       // console.log('safeFlag:'+safeFlag)
        ...
        this.OPCODES = {}
        this.rpc_api_util = new RPCApiUtil(null)
        this.initFuncs()
    }
    //切换protocol
    ...
    jsrun(context,opcode,...etc)//token_x,opval,extra_data,
    {
        ...
        if(etc.length>=1)//>=2
        {
           ...
           // let opret = etc.pop();
            ...
            // ret = context[opret]
            ret = parent[opList[opList.length-1]](...params)
        }else{
            ret = parent[opList[opList.length-1]]()//popFunc()//etc.length>0 ? popFunc( context[etc[0]]):
        }
        context['$ret'] = ret
        return ret
    }
    ...
    async op(context,opcode,...etc){
        ...
        try{
            ...
            if((opcode).startsWith('/') || (opcode).startsWith('http://') || (opcode).startsWith('https://')) {//这个是查询函数的绑定（也是直接解析的后端查询）
                ...
                let url = new URL((opcode).startsWith('/')  ?'http://127.0.0.1:9000'+opcode:opcode)//利用上这个解析库进行查询。
    ...
    async opbatch(context,list,iswait=true)
    {
        ...
        try{
            ...
            for(let i=0;i<list.length;i++)
            {
                ...
                if($log_result_flag) results.push(deepClone(ret))//(deepcopy(ret))// this.deepCopy(ret) )//进行了结果对象的复制--否则会导致stringify拿到的一般情况下是context的重复
