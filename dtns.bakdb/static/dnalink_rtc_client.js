...
async function queryTurnPwd(mywallet,roomid = null,tns_server_url = null)
{
    ...
    console.log('hash:',hash,time_x)
     window.g_turn_pwd = await
        http_req.http_post((tns_server_url ?tns_server_url : get_TNS_SERVER_URL())+'/c',//?timestamp='+time_x+'&sign='+encodeURIComponent(time_sign_base64), 
            {
                timestamp:time_x ,
                sign: sign,
...
class RTCClient
{
    constructor(roomid,tns_server_url,options,isLocalReq = true)
    {
        ...
        let This = this
        this.setWallet(g_forklist_user,function(){
            This.init()
        })
    }
    ...
    onPeer(peer) {
        try{
            ...
            peer.on('connect', () => {
                console.log('peer is connected',typeof This.peerRefreshCallback )
                This.peerMap.set(peer.channelName,peer)
    ...
    async download(params,end){
        ...
        // console.log('peers()-:'+this.client.peers())
        // if(!peerNow) this.peer() //this.client.peers().forEach(peer => {peerNow=peer})
        if(!peerNow) return null
        ...
        while(params)
        {
            ...
            if(data.data && data.data.msg && !data.data.ret)
            {
                end(data)
                break;
            }
            console.log('fileInfo-download-data:',data)
            if(!data.fileInfo || data.data.length!=len && begin+len<=data.fileInfo.size)
            {
                unfast_mode = true
                continue
            }
