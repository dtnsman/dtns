 ...
 */
window.clouddisk_c = {}
const clouddisk_c_token_name = OBJ_TOKEN_NAME
const clouddisk_c_api_base   = OBJ_API_BASE
const clouddisk_c_token_root = OBJ_TOKEN_ROOT
 ...
 * 查询我的文件夹
 */
clouddisk_c.queryMyFolder = queryMyFolder
async function queryMyFolder(req,res)
{
    ...
    let folderInfo  = await rpc_api_util.s_query_token_info(clouddisk_c_api_base,userInfo.folder_id,'assert')
    if(!folderInfo) res.json({ret:false,msg:'query folder-info is empty'})
    return res.json({ret:true,msg:'success',folderInfo})
}
//创建文件夹
clouddisk_c.createFolder = createFolder
...
clouddisk_c.saveFile2Folder = saveFile2Folder
async function saveFile2Folder(req,res)
{
 ...
 */
clouddisk_c.listFolderFiles = listFolderFiles
async function listFolderFiles(req,res)
{
