...
async function bindDevice(req,res)
{
    ...
    //#0 先行验证
    let deviceHash = await key_util.hashVal(deviceName)// eccryptoJS.sha256(eccryptoJS.utf8ToBuffer(deviceName))
...
async function loginDevice(req,res)
{
    ...
    if(dtns_device_id.split('_')[1].indexOf(web3name)!=0 && web3name!='dtns' && web3name.indexOf('dev')<0)  
...
{
    let {user_id,s_id,web3name,web3desc,name,logo,pics,pubkey,labels} = str_filter.get_req_data(req);
