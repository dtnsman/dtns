...
async function queryTurnPwd(tns_server_url = null)
{
    if(ll_config.roomid=='dtns')
    {
        ...
        http_req.http_post((tns_server_url ?tns_server_url : get_RTCCLIENT_CONFIG_TNS_SERVER_URL())+'/c',//?timestamp='+time_x+'&sign='+encodeURIComponent(time_sign_base64), 
            {
                timestamp:time_x ,
        ...
        var time_sign_base64 = null;// time_hash.toString(CryptoJS.enc.Base64)
        console.log('time_sign_base64:',time_sign_base64,time_x)
        window.g_turn_pwd = await
        http_req.http_post((tns_server_url ?tns_server_url : get_RTCCLIENT_CONFIG_TNS_SERVER_URL())+'/c',//?timestamp='+time_x+'&sign='+encodeURIComponent(time_sign_base64), 
            {
