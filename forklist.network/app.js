...
async function queryUrls()
{
    ...
    http_req.http_get( typeof g_static_dtns_network_nslookup_url!='undefined' ? g_static_dtns_network_nslookup_url : 'https://static.yunapi.org/static-dtns-network.json')//?timestamp='+time_x+'&sign='+encodeURIComponent(time_sign_base64), 
...
process.on('uncaughtException', function(err) {
    ...
    let str = ''+err
    console.log('uncaughtException-str:',str)
    if(str && str.indexOf('cannot signal after peer is destroyed')>=0){
        console.log('[Exit]catch-exit-exception------process.exit(1)')
        process.exit(1);
    }
    ErrorLog.write(err);
...
process.on('unhandledRejection', (reason, p) => {
    p.catch((err) => {
        console.log(err);
        console.log(err ? err.stack:null);
