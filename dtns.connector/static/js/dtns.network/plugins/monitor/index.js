...
function countMinMonitorPeer(req)
{
    ...
    if(cntObj.cnt>max_cnt) //如果超过频率，req.peer会被锁定
    {
        ...
        req.peer.send(JSON.stringify({notify_type:'need_captcha',channel:'monitor-agent',user_id:'system-user',xobj:{max_cnt,cnt:cntObj.cnt,locked:true}}))
    }
...
async function sendMonitorAgentChannelMsg(xobj,notify_type = 'monitor-agent')
{
    let user_id = 'monitor-system-default-user'
    let channel = 'monitor-agent'
    xobj = !xobj ? {}:xobj
...
{
    let rtmarketUser = user_id?user_id: 'monitor-system-default-user'
    let defaultPeer = peer ? peer:{send:recvMonitorClientUserChannelMsg}
    let channel = 'monitor-agent'
    let ret = user_id?{ret:true}: await new Promise((res)=>
    {
        rtchannel_c.create({params:{channel,user_id:rtmarketUser},peer:defaultPeer},{json:function(data){
            res(data)
        }})
    })
    ...
    {
        rtchannel_c.focus({params:{channel,user_id:rtmarketUser},peer:defaultPeer},{json:function(data){
            res(data)
        }})
    })
    console.log('creatMonitorAgentChannel-focus:',ret)
    if(ret && ret.ret) return true
    return false
}