...
const defaultOptions = { wrtc,forceNew:true,reconnection: true,timeout:3000,rejectUnauthorized: false,
    transports: ['polling', 'websocket'],config:iceconfig,stream: false,streams: [],
    //channelConfig:{ordered:false} 
 }

class RTCClient
{
    constructor(roomid,tns_server_url=null,options=null,peerPoolCnt=1)
    {
    ...
    init()
    {
        ...
        this.client.on('discover', (discoveryData) => {
            ...
            if(discoveryData.hostid){ 
                ...
                //多连几个（设置一个peer池，仿http请求）
                for(let i=0;i<This.peerPoolCnt-1;i++)
                setTimeout(()=>{
                    This.connectToPeer(discoveryData.hostid)
                },1000*i)
                // setTimeout(()=>{
    ...
    async download(params,end){
        ...
        let rpc_name = params.rpc_name
        console.log('download-rpc_name:'+rpc_name)
        while(params)
        {
            // if(!this.fastPeer)
            {
            ...
            {
                This.sendData(peerNow,rpc_name+'::/op',params,null,function(readData){
                    resolve(readData)
                })
                //setTimeout(()=>resolve(null),unfast_mode ? 60000:6000)
            })
            console.log('download-from-dnalink-engine----data-fileInfo:'+JSON.stringify(data.fileInfo)+' rpc_name:'+rpc_name)
            if(!peerNow._pc) {
    ...
    sendFile(data,fileInfo,callback){
        let peerNow = this.fastPeer
        if(!peerNow) this.client.peers().forEach(peer => {peerNow=peer})
        ...
        {
            console.log('data.data.params:'+JSON.stringify(data))
            if(data && data.data && data.data.params) delete data.data.params //清理旧这个可能的问题params（会泄露dnalink.engine的密钥信息）
            if(typeof callback=='function') callback(data)
        }

        if(fileInfo.encoding!='fromfile_binary')
            this.sendData(peerNow,data.rpc_name+'::/op',rdata,fileInfo,func)
        else
            this.sendDataBinary(peerNow,data.rpc_name+'::/op',rdata,fileInfo,func)//sendDataBinaryD
    }
...
async function test()
{
    let client = new RTCClient('linkline')
    //client.sendData()
    ...
    // else
    {
        ...
        data:{appid:10001,secret_key:'a6535c84e33d97777aa7dafbb4212322',opcode:'fork',
        ...
        obj.url='59872::/chain/query'//'/chain/token'//'80::/chain/token'
        //obj.data.rpc_name='59872'
