...
const rp = xrequire('request-promise');
...
async function req_best_move_callback(chess_id,result,user_id = 'user_rb7fEHhdQwUXJExW')
{
    ...
    //请求动作---进行下一步操作的请求
    let ret = await robot_http_get('https://www.chessdb.cn/chessdb.php?action=queryall&learn=1&showall=0&board='+result.user_pen_code.replace(' ','%20'))
    console.log('robot-result:',ret)
    if(true || !ret || ret.indexOf('move:')<0)
    {
        ret = await robot_best_move2(result.user_pen_code)
    }
...
async function creatRTChessAgentChannel(agent_user_id = 'user_8wSeyn7qNQsQEszf,user_9v8EKyJSjwgg3P1a')
{
    let user_id = rtchess_channel_system_user_id
    let channel = rtchess_channel_agent_name
    let ret = await new Promise((res)=>
